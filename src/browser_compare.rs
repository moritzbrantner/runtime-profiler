...
use crate::chromium_trace::{ChromiumTraceSummary, CHROMIUM_TRACE_SUMMARY_SCHEMA_V1};
...
fn assess_recorded_identity(
    ...
    match (reference_runtime, candidate_runtime) {
        (Some(reference), Some(candidate)) => compare_runtime_identity(
            reference,
            candidate,
            &mut mismatches,
            &mut missing,
...
fn compare_runtime_identity(
    ...
    let reference_viewport = format!(
        "{}x{}",
        reference.viewport.width, reference.viewport.height
    );
    let candidate_viewport = format!(
        "{}x{}",
        candidate.viewport.width, candidate.viewport.height
    );
    compare_required(
        "browser viewport",
...
mod tests {
    ...
    fn summary() -> ChromiumTraceSummary {
        ChromiumTraceSummary {
            ...
            trace_event_count: 0,
            main_thread: crate::chromium_trace::ChromiumThreadIdentity {
                process_id: 1,
                thread_id: 2,
